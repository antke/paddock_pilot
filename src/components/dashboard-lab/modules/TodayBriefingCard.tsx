...
export function TodayBriefingCard({
  ...
  const events = (
    data.todayEvents.length > 0 ? data.todayEvents : data.events
  ).slice(0, eventLimit)
  ...
  return (
    <section
      className={dashboardSectionClassName(
        chrome,
        chrome === 'cards' ? 'border-primary/15 bg-card/85 md:p-6' : undefined,
      )}
    >
      <div
        className={
        ...
        <div className="grid gap-4">
          <div>
            ...
            <p
              className={
                showNextEvent
                  ? 'mt-3 max-w-3xl text-balance text-base leading-7 text-muted-foreground md:text-lg'
                  : 'mt-3 text-balance text-base leading-7 text-muted-foreground md:text-lg'
              }
            >
              {briefingText}
            </p>
      ...
      {showTimeline && (
        <div className="mt-6 grid gap-3">
          {events.length === 0 ? (
            ...
            events.map((event) => (
              <EventLinkCard key={event._id} event={event} chrome={chrome} />
            ))
          )}
        </div>
      )}
...
function NextEventPanel({
  ...
  event: DashboardLabEvent | undefined
  chrome: DashboardLabChrome
}) {
  if (!event) {
    return (
      <div
        className={dashboardInlinePanelClassName(
          chrome,
          chrome === 'cards' ? 'border-dashed bg-muted/25 p-4' : undefined,
        )}
      >
  ...
  return (
    <Link
      ...
      className={dashboardInlinePanelClassName(
        chrome,
        'group/next bg-primary/8 transition-colors hover:bg-primary/12 focus-visible:ring-2 focus-visible:ring-ring/25 focus-visible:outline-none',
      )}
    >
      <p className="text-3xl font-semibold tracking-tight text-primary">
        {event.time}
      </p>
      <p className="mt-2 font-semibold underline-offset-4 group-hover/next:underline">
        {event.title}
      </p>
      {event.location && (
...
function createBriefingText(
  data: DashboardLabData,
  nextEvent: DashboardLabEvent | undefined,
) {
