...
const timelineDayFormatter = new Intl.DateTimeFormat('en-GB', {
  day: 'numeric',
  month: 'long',
  year: 'numeric',
})

const timelineShortDayFormatter = new Intl.DateTimeFormat('en-GB', {
  day: 'numeric',
  month: 'short',
})
...
export function createLabAnalysis(
  ...
  return {
    metrics: [
      ...
      },
      {
        ...
        tone:
          data.overview.summary.activeMedicationCount > 0 ? 'urgent' : 'default',
      },
      {
        title: 'Upcoming care',
      ...
      },
      {
        ...
        tone:
          completionCoverage.eventNoteCoveragePercent < 75 ? 'urgent' : 'steady',
      },
    ],
    summary: {
      urgentCount,
...
function compareEventDateAndTime(
  a: DashboardLabEvent,
  b: DashboardLabEvent,
) {
...
function compareTimelineSignal(
  a: LabTimelineSignal,
  b: LabTimelineSignal,
) {
  const dateSort = a.date.localeCompare(b.date)
...
function getTimeline(
  ...
  for (const occurrence of occurrences) {
    let dayKey = occurrence.startDate < windowStart ? windowStart : occurrence.startDate
    const finalDayKey = occurrence.endDate > windowEnd ? windowEnd : occurrence.endDate
  ...
  const buckets = dayKeys.map((dayKey) => {
    ...
    const healthSignals = bucketSignals.filter((signal) => signal.kind === 'health')
    ...
    return {
      ...
      medicationCount: bucketSignals.filter((signal) => signal.kind === 'medication')
        .length,
      reminderCount: bucketSignals.filter((signal) => signal.kind === 'reminder')
        .length,
      nutritionChangeCount: bucketSignals.filter(
        (signal) => signal.kind === 'nutrition',
      ).length,
      weightRecordCount: bucketSignals.filter((signal) => signal.kind === 'weight')
        .length,
  ...
  const busiestBucket = buckets.reduce<LabTimelineBucket | null>(
    (busiest, bucket) => {
      if (!busiest || getBucketActivityCount(bucket) > getBucketActivityCount(busiest)) {
        return bucket
      }
...
function getUniqueOccurrenceEvents(
  occurrences: Array<LabTimelineOccurrence>,
) {
...
function formatDayLabel(dayKey: string) {
  return timelineDayFormatter.format(parseDayKey(dayKey))
}

function formatShortDayLabel(dayKey: string) {
  return timelineShortDayFormatter.format(parseDayKey(dayKey))
}
...
function compareAttentionHorse(
  a: LabAttentionHorse,
  b: LabAttentionHorse,
) {
  const urgentSort =
  ...
  const activeSort =
    b.activeIssueCount + b.activeMedicationCount -
...
function getHorseProfileItems(
  ...
  return horses.map((horse) => {
    ...
    return {
      ...
      coveragePercent: Math.round((completedFieldCount / profileFieldCount) * 100),
    }
  })
}
