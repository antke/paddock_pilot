...
export function getTimelinePeriods(
  ...
  if (scale === 'day') {
    return buckets.map((bucket) => createTimelinePeriod(scale, bucket.key, bucket.key, [bucket]))
  }
...
function compareSignal(a: LabTimelineSignal, b: LabTimelineSignal) {
  ...
  const kindSort = getSignalKindOrderIndex(a.kind) - getSignalKindOrderIndex(b.kind)
...
function getPeriodLabel(
  ...
  if (scale === 'week') return `Week of ${formatLongDate(startKey)}`
  if (scale === 'month') return monthFormatter.format(parseDayKey(startKey))
  return formatLongDate(startKey)
}
...
function getPeriodShortLabel(
  ...
  if (scale === 'month') return shortMonthFormatter.format(parseDayKey(startKey))
  if (startKey === endKey) return shortDateFormatter.format(parseDayKey(startKey))
  return `${shortDateFormatter.format(parseDayKey(startKey))}–${shortDateFormatter.format(parseDayKey(endKey))}`
}
...
function addDaysKey(dayKey: string, days: number) {
  return formatDateKey(new Date(parseDayKey(dayKey).getTime() + days * oneDayInMs))
}
